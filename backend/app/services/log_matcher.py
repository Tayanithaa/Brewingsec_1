...
def _denorm_backslashes(value: str) -> str:
    """Collapses doubled backslashes down to single ones. Windows-path values
    are a common source of accidental double-escaping (both in hand-written
    rules and in generated reference data — this exact bug was found and
    fixed in this dataset's reference rules). Applying this uniformly across
    every modifier, not just |endswith, means a rule author's escaping
    mistake degrades gracefully instead of silently matching nothing."""
...
def _field_matches(entry: dict, field_expr: str, expected: Any) -> bool:
    ...
    if modifier is None:
        ...
        actual_norm = _denorm_backslashes(_normalize(actual))
        return any(actual_norm == _denorm_backslashes(_normalize(opt)) for opt in options)

    actual_str = _normalize(actual).lower()
    actual_str_norm = _denorm_backslashes(actual_str)

    if modifier == "contains":
        return any(_denorm_backslashes(_normalize(opt).lower()) in actual_str_norm for opt in options)
    if modifier == "startswith":
        return any(actual_str_norm.startswith(_denorm_backslashes(_normalize(opt).lower())) for opt in options)
    if modifier == "endswith":
        return any(actual_str_norm.endswith(_denorm_backslashes(_normalize(opt).lower())) for opt in options)
    if modifier == "re":
        for opt in options:
            try:
