...
# Split benign across the 3 event types so no single type dominates
BENIGN_LSASS_ACCESS = round(BENIGN_TOTAL * 0.35)   # normal process access to lsass.exe (AV/EDR)
BENIGN_POWERSHELL = round(BENIGN_TOTAL * 0.35)     # normal powershell process creation
BENIGN_REGISTRY = BENIGN_TOTAL - BENIGN_LSASS_ACCESS - BENIGN_POWERSHELL  # registry padding, absorbs rounding
