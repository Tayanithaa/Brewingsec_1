...
export default function App() {
  ...
  const loginFlow = useCallback(async () => {
    try {
      const res = await login(`user_${Math.random().toString(36).substring(7)}`);
      ...
      <main className="flex-1 p-6 z-10 overflow-y-auto custom-scrollbar">
        ...
        {activeTab === 'sandbox' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full items-stretch">
            ...
            <div className="lg:col-span-3 flex flex-col space-y-4 border border-gray-800 rounded bg-surface/50 overflow-hidden h-full">
              <div className="bg-black px-4 py-3 border-b border-gray-800 flex justify-between items-center">
                <h3 className="font-mono text-xs text-textSecondary uppercase tracking-widest glow-text-blue">Match Results</h3>
                <span className="text-[10px] bg-primary/20 text-primary px-2 py-0.5 rounded font-mono border border-primary/30">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-0">
                {runResults ? (
                  <LogPanel entries={runResults.matched_entries} />
                ) : (
                  <div className="h-full flex flex-col items-center justify-center text-gray-600 space-y-3 px-4 text-center">
                    <span className="text-3xl opacity-50">📡</span>
                    <p className="font-mono text-xs">Run a rule against a dataset to view matched log entries.</p>
                  </div>
                )}
              </div>
              ...
              {runResults && (
                <StatsBar 
                  total={runResults.total_entries} 
                  matched={runResults.match_count} 
                  fpRate={runResults.fp_rate_estimate} 
                />
              )}
            </div>
          </div>
