...
export const getUserProgress = async (token) => {
  const response = await axios.get(`${API_URL}/user/progress`, getAuthHeaders(token));
  return response.data;
};

export const getLeaderboard = async (token) => {
  const response = await axios.get(`${API_URL}/leaderboard`, getAuthHeaders(token));
  return response.data;
};

export const unlockHint = async (challengeId, hintIndex, token) => {
  const response = await axios.post(`${API_URL}/challenges/${challengeId}/hints/${hintIndex}/unlock`, {}, getAuthHeaders(token));
  return response.data;
};
